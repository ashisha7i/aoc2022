...
public class Day5 {
    ...
    public static Stack<Character>[] createStacks(List<String> crates) {
        ...
        Arrays.setAll(stacks, element -> new Stack<>());
    ...
    private static void performInstructions(Stack<Character>[] stacks) {
        for(String action : instructions)
            moveBoxes(Delivery.parse(action), stacks);

    }

    public static void moveBoxes(Delivery delivery, Stack<Character>[] stacks) {
        List<Character> boxes = new ArrayList<>();
        for(int i = 0; i < delivery.number(); i++)
            boxes.add(stacks[delivery.fromIndex()].pop());

        for(Character b : boxes)
            stacks[delivery.toIndex()].push(b);
